...
const DragAssign: React.FC<any> = () => {
    ...
    const handleMultiple = (index: number, event?: React.MouseEvent<HTMLButtonElement>) => {
        setTasks((prevTasks) => {
            ...
            if (newTasks[index]?.checked) {
                ...
                setElementCount((prevCount) => prevCount + (selectedAssignees.length > 1 ? selectedAssignees.length : 1));
    ...
    return (
        ...
        >
            ...
            <button className="add-button" onClick={handleSubmit} disabled={submitting === true} style={{ margin: '14px' }}>
                Send
            </button>
        </div>
    );
