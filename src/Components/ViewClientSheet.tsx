...
const ViewClientSheet: React.FC<any> = () => {
  ...
  const [editEveningCommentMode, setEditEveningCommentMode] = useState<
    number | null
  >(null);
  const [editedMorningComments, setEditedMorningComments] = useState<
    Record<string, string>
  >({});
  const [editedEveningComments, setEditedEveningComments] = useState<
    Record<string, string>
  >({});
  ...
  const [buttonClick1, setButtonClick1] = useState<number>(0);
    ...
    axios
      ...
      .then((response) => {
        ...
        const uniqueAssignees: string[] = Array.from(
          new Set([
            ...(response.data.data as ClientSheetData[]).map(
              (item) => item?.AssigneeName
            ),
            ...backlog.map((item: any) => item?.assigneeName || ""),
          ])
        ...
        if (dateRange[0] && dateRange[1]) {
          ...
          filteredData = response.data.data.filter(
            (item: any) => item.created_at.split("T")[0] === formattedDate
          );
        }
  ...
  const filteredData = data.filter((project) => {
    if (employeeID === "B2B00100" && project.taskRemainder === 0) {
      ...
      const isAssignedByEmployee =
        project.assignedBy === employeeName && project.taskRemainder === 0;
      const isAssignedToEmployee =
        project.AssigneeName === employeeName && project.taskRemainder === 0;
      return (
        (isAssignedByEmployee || isAssignedToEmployee) &&
        project.assignedBy !== "" &&
        project.AssigneeName !== "" &&
  ...
  const filteredRemainder = data.filter((project) => {
    if (employeeID === "B2B00100" && project.taskRemainder === 1) {
      ...
      const isAssignedByEmployee =
        project.assignedBy === employeeName && project.taskRemainder === 1;
      const isAssignedToEmployee =
        project.AssigneeName === employeeName && project.taskRemainder === 1;
      return (
        (isAssignedByEmployee || isAssignedToEmployee) &&
        project.assignedBy !== "" &&
        project.AssigneeName !== "" &&
  ...
  //act time
  const entriesWithActTime = filteredData.filter(
    (entry) => entry.actTime !== null && entry.actTime !== ""
  );
  const actTimeStrings = entriesWithActTime.map((entry) => entry.actTime);
  const actTimeArrays = actTimeStrings.map((timeString) =>
    timeString.split(":").map(Number)
  );
  const totalMinutes = actTimeArrays.reduce(
    (sum, [hours, minutes]) => sum + hours * 60 + minutes,
    0
  );
  ...
  //est time
  const entriesWithEstTime = filteredData.filter(
    (entry) => entry.estTime !== null && entry.estTime !== ""
  );
  const estTimeStrings = entriesWithEstTime.map((entry) => entry.estTime);
  const estTimeArrays = estTimeStrings.map((timeString) =>
    timeString.split(":").map(Number)
  );
  const totalEstMinutes = estTimeArrays.reduce(
    (sum, [hours, minutes]) => sum + hours * 60 + minutes,
    0
  );
  ...
  // act time
  const entriesWithAct = filteredRemainder.filter(
    (entry) => entry.actTime !== null && entry.actTime !== ""
  );
  const actTimeString = entriesWithAct.map((entry) => entry.actTime);
  const actTimeArray = actTimeString.map((timeString) =>
    timeString.split(":").map(Number)
  );
  const totalMinute = actTimeArray.reduce(
    (sum, [hours, minutes]) => sum + hours * 60 + minutes,
    0
  );
  ...
  //est time
  const entriesWithEstTimes = filteredRemainder.filter(
    (entry) => entry.estTime !== null && entry.estTime !== ""
  );
  const estTimeString = entriesWithEstTimes.map((entry) => entry.estTime);
  const estTimeArray = estTimeString.map((timeString) =>
    timeString.split(":").map(Number)
  );
  const totalEstMinute = estTimeArray.reduce(
    (sum, [hours, minutes]) => sum + hours * 60 + minutes,
    0
  );
  ...
  const handleMorningCheckChange = (record: ClientSheetData) => {
    const updatedData = data.map((item) =>
      item.id === record.id
        ? { ...item, morningCheck: item.morningCheck === 1 ? 0 : 1 }
        : item
    );
  ...
  const handleEveningCheckChange = (record: ClientSheetData) => {
    const updatedData = data.map((item) =>
      item.id === record.id
        ? { ...item, eveningCheck: item.eveningCheck === 1 ? 0 : 1 }
        : item
    );
  ...
  const handleMorningCommentChange = (
    record: ClientSheetData,
    value: string
  ) => {
  ...
  const handleEveningCommentChange = (
    record: ClientSheetData,
    value: string
  ) => {
  ...
  const handleSaveComments = (record: ClientSheetData) => {
    ...
    const morningComment =
      editedMorningComments[id] || record.morningComment || "";
    const eveningComment =
      editedEveningComments[id] || record.eveningComment || "";

    const updatedData = data.map((item) =>
      item.id === record.id ? { ...item, morningComment, eveningComment } : item
    );
    setData(updatedData);
    axios
      .put(`${process.env.REACT_APP_API_BASE_URL}/update-comments/${id}`, {
        morningComment,
        eveningComment,
      })
      .then((response) => {
  ...
  const handleActTimeChange = (id: number, value: string) => {
    ...
    setData(updatedData);
    axios
      .put(`${process.env.REACT_APP_API_BASE_URL}/update-act-time/${id}`, {
        actTime: value || "",
      })
      .then((response) => {
  ...
  const loadData = () => {
    setButtonClick((prev) => prev + 1); // Update buttonClick state to trigger useEffect
  };
  const loadDataTable = () => {
    setButtonClick1((prev) => prev + 1); // Update buttonClick state to trigger useEffect
  };
  const columns = [
    {
      title: "Project Name",
          ...
          {editMode === record.id ? (
            <>
              <textarea
                ...
                onChange={(e) =>
                  handleMorningCommentChange(record, e.target.value)
                }
              />
          ...
          {editEveningCommentMode === record.id ? (
            <>
              <textarea
                ...
                onChange={(e) =>
                  handleEveningCommentChange(record, e.target.value)
                }
              />
              <Button onClick={() => handleSaveComments(record)}>Save</Button>
            </>
            ...
            <>
              <div>{text}</div>
              <Button
                onClick={() => setEditEveningCommentMode(record.id)}
                style={{ float: "right" }}
              >
                Edit
              </Button>
