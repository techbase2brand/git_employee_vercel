...
const ViewTlBacklog: React.FC<Props> = ({
  searchTerm,
  dateRange,
  selectedAssignee,
  setBacklog,
  ...
  useEffect(() => {
    ...
    if (selectedAssignee) {
      filteredData = originalData.filter(
        (item) => item?.assigneeName === selectedAssignee
      );
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<BacklogTask[]>(
          `${process.env.REACT_APP_API_BASE_URL}/get/BacklogTasks`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("myToken")}`,
            },
        ...
        if (employeeName === "Managing Director") {
          const filteredData = sortedData?.filter((e) => {
            e.UserEmail === UserEmail;
            const assigneeMatch = e.assigneeName
              .toLowerCase()
          ...
          const finalFilteredData = filteredData?.filter(
            (e) => e.faChecked === null || e.faChecked === 0
          );
          setData(finalFilteredData);
        } else {
          const finalFilteredData = filteredData?.filter((e) => {
            const isAssignedByAdmin =
              e.UserEmail === UserEmail || e.assigneeName === myName;
            return isAssignedByAdmin;
          });
          const finalFilteredData2 = finalFilteredData?.filter(
            (e) => e.faChecked === null || e.faChecked === 0
          );
    ...
    fetchData();
  }, [adminID, dateRange, searchTerm, buttonClick1]);
  ...
  const handleModalOk = () => {
    ...
    const updatedData = data.filter(
      (item) => item.backlogTaskID !== modalRecord?.backlogTaskID
    );
    setData(updatedData);
    axios
      .put(
  ...
  const handleCheckboxChange = (isChecked: boolean, backlogTaskID: number) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE_URL}/update/task-completion/${backlogTaskID}`,
